...
var Range = function (start, end, step) {
    ...
    this.start = start;
    this.end = end==0?end: (end || this.start)
    this.step = step || 1
};

Range.prototype.loop = function(callback){
    var reverse = this.start > this.end?true : false
    var start = reverse? this.end : this.start
    var end = reverse? this.start:this.end
...
Range.prototype.size = function () {
    var size = 0;
    this.loop((number) => size++)
    return size
};

Range.prototype.each = function (callback) {
    this.loop(one => callback(one))
};

Range.prototype.includes = function (val) {
    var found = false
     this.loop((number) => found = number == val? true : found)
     return found
};

var range1 = new Range(1);

var range = new Range(10,0, -2);
console.log(range)
console.log(range.includes(1), range.includes(2))
