...
var Tree = function(value) {
  ...
  Tree.prototype.map = function(callback, tree=null) {
        if(tree){
            tree.children.push(new Tree(callback(this.value)))
            this.children.forEach((one, i) =>{
                one.map(callback, tree.children[i])
            })
        } 
        else{
            tree =  new Tree(callback(this.value))
            this.children.forEach((one, i) =>{
                one.map(callback, tree)
            })
  ...
  var tree = new Tree(5)
  tree.children.push(new Tree(8))
  tree.children.push(new Tree(9))
  tree.children[0].children.push(new Tree(12))





  console.log(tree.map(one => one * one), tree)